...
//int brightness = 10;    // how bright the LED is
 ...
 int rajzoltC  = (tempC - 98 ) * -1 ;// 100 fok -100= 0 tkarunk ekorát ki 50 fok-100= 50 et tkarunk ki *-1 hogy poztiv legyen
...
//speedinkm = String(f);
 ...
 void smartDelay(int long ms)
{
  int long start = millis();
  do 
  {
    while (ss.available())
      gps.encode(ss.read());
